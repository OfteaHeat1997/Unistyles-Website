...
async function getCremasCategoryId() {
    ...
    try {
        const result = await db.query(
            "SELECT id FROM categories WHERE slug = 'cremas' LIMIT 1"
        );
  ...
  try {
    ...
    const query = `
      SELECT
        ...
        p.image, p.legacy_image, p.skin_type, p.skin_concern,
...
router.get('/filters', asyncHandler(async (req, res) => {
  try {
    ...
    const concernsQuery = `
      SELECT skin_concern AS concern, COUNT(*) AS count
      FROM products p
      WHERE p.category_id = $1
    ...
    const areasQuery = `
      SELECT application_area AS area, COUNT(*) AS count
      FROM products p
      WHERE p.category_id = $1
