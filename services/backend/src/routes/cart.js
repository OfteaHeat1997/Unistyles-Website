...
router.post('/add',
    ...
    const product = await db.query(
        'SELECT id, name, price, in_stock, stock_quantity, image, legacy_image FROM products WHERE id = $1',
        [productId]
    );
...
async function enrichCartItems(cartItems) {
    ...
    const products = await db.query(`
        SELECT id, ref, name, slug, price, image, legacy_image, in_stock, stock_quantity
        FROM products
        WHERE id = ANY($1)
    `, [productIds]);
