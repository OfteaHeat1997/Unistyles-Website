    ...
    asyncHandler(async (req, res) => {
        ...
        try {
            ...
            const products = await client.query(
                'SELECT id, ref, name, price, in_stock, stock_quantity, legacy_image FROM products WHERE id = ANY($1) AND published_at IS NOT NULL FOR UPDATE',
                [productIds]
            );
            ...
            const freeDeliveryThreshold = parseFloat(settingsResult.rows[0]?.value || 0);
            const deliveryFeeResult = await client.query("SELECT value FROM settings WHERE key = 'delivery_fee'");
            const standardDeliveryFee = parseFloat(deliveryFeeResult.rows[0]?.value || 0);
