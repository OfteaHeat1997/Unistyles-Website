...
// Professional Fragrance E-commerce API
// ===========================================
  ...
  try {
    ...
    const query = `
      SELECT DISTINCT
        ...
        p.legacy_image as image, p.gender, p.fragrance_family,
...
router.get('/filters', asyncHandler(async (req, res) => {
  try {
    ...
    const familiesQuery = `
      SELECT DISTINCT fragrance_family as family, COUNT(*) as count
      FROM products p
      JOIN products_category_links pcl ON p.id = pcl.product_id
...
router.get('/', asyncHandler(async (req, res) => {
  ...
  // Products query - include all ORDER BY columns in SELECT for DISTINCT
  const productsQuery = `
    SELECT DISTINCT
      p.id, p.name, p.slug, p.description, p.price,
